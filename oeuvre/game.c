...
int main(int argc, char **argv)
{
    ...
    joueur = load_texture_from_image("./joueur.png", window, renderer); //charger l'image du joueur
    Fond = load_texture_from_image("./str.jpg", window, renderer);  //charger l'image du background
    ...
    while (end == 0 && vie > 0)
    {
        ...
        if (intersection == 1)    //une collision a eu lieu
        {
            vie -= intersection;
        }
     ...
     SDL_Window *window, SDL_Renderer *renderer, int *intersection, int vie, int temps)
{
    ...
    while (program_on && (i < nb_it) && (end == 0))
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            case SDL_KEYDOWN:   //appuie sur une touche
                ...
                case SDLK_DOWN:   //fleche bas
                    ...
                    break;
                case SDLK_q:    // le jeu s'arrete si le joueur appuie sur q
                    end = 1;
                    break;
            ...
            case SDL_QUIT:     //le jeu s'arrete par fermeture de la fenetre
                end = 1;
                break;
            default:
                break;
        ...
        *intersection = collision(&destination, destination2);   //gestion de collision
...
//gcc game.c  -lSDL2_image  -lSDL2_ttf -o game $(sdl2-config --cflags --libs)