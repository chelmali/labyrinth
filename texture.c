...
//afficher une texturesur la totalité de l'ecran
...
void play_with_texture_3(SDL_Texture* my_texture,
  ...
  for (int i = 0; i < nb_it; ++i) {
    ...
    SDL_SetTextureAlphaMod(my_texture,(1.0-1.0*i/nb_it)*255); 
         // L'opacité va passer de 255 à 0 au fil de l'animation
...
void play_with_texture_4(SDL_Texture* my_texture, SDL_Texture* bg_texture,
       ...
       int speed = 9;
       for (int x = 0; x < (window_dimensions.w - destination.w)/2; x += speed) {
        ...
        play_with_texture_1_1(bg_texture,window,renderer);     // Effacer l'image précédente avant de dessiner la nouvelle
         SDL_RenderCopy(renderer, my_texture, // Préparation de l'affichage
                        &state,
                        &destination);  
         SDL_RenderPresent(renderer);         // Affichage
       ...
       state.x = 0 ;                          // La première vignette est en début de ligne
       state.y = 1 * offset_y;                // On s'intéresse à la 4ème ligne, le bonhomme qui court
       state.w = offset_x;                    // Largeur de la vignette
       state.h = offset_y;    

       for (int x = (window_dimensions.w - destination.w)/2; x < ((window_dimensions.w - destination.w)/2)+(window_dimensions.w - destination.w)/20; x += speed) {
         destination.x = x;                   // Position en x pour l'affichage du sprite
         state.x += offset_x;                 // On passe à la vignette suivante dans l'image
         state.x %= source.w; 
         destination.y =                        // La course se fait en milieu d'écran (en vertical)
         (window_dimensions.h - destination.h)/3;
         SDL_RenderClear(renderer); 
        play_with_texture_1_1(bg_texture,window,renderer);     // Effacer l'image précédente avant de dessiner la nouvelle
         SDL_RenderCopy(renderer, my_texture, // Préparation de l'affichage
                        &state,
      ...
      state.x = 0 ;                          // La première vignette est en début de ligne
       ...
       for (int x = ((window_dimensions.w - destination.w)/2)+(window_dimensions.w - destination.w)/20; x < (window_dimensions.w - destination.w); x += speed) {
         destination.x = x;                   // Position en x pour l'affichage du sprite
         state.x += offset_x;                 // On passe à la vignette suivante dans l'image
         state.x %= source.w; 
         SDL_RenderClear(renderer); 
...
SDL_Texture*  my_texture = load_texture_from_image("labyrinthe.jpg",window, renderer );
...
my_texture = load_texture_from_image("labyrinthe.gif",window, renderer );
 ...
 my_texture = load_texture_from_image("sun.png",window, renderer );
SDL_Texture* bg_texture = load_texture_from_image("basket.jpg",window, renderer );
play_with_texture_3( my_texture,bg_texture, window, renderer);

 my_texture = load_texture_from_image("sprite.png",window, renderer );
 bg_texture = load_texture_from_image("route.jpg",window, renderer );
play_with_texture_4( my_texture,bg_texture, window, renderer);

 my_texture = load_texture_from_image("shadow.png",window, renderer );
 bg_texture = load_texture_from_image("nasa.jpg",window, renderer );
